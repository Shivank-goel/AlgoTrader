...
function render(d) {
  ...
  const sim=s.simulation||{}, eq=s.equity||{}, allPositions=[...(s.positions||[]).map(p=>({...p,side:'LONG'})),...(s.short_positions||[]).map(p=>({...p,side:'SHORT'}))], trades=s.trade_forward_observations||[];
  document.getElementById('simulation-id').textContent=sim.simulation_id||'—'; document.getElementById('starting-capital').textContent=money(sim.initial_capital);
  document.getElementById('shadow-equity').textContent=money(eq.current_equity||val.equity); document.getElementById('shadow-cash').textContent=money(account.cash); document.getElementById('shadow-realized').textContent=money(eq.realized_pnl??account.realized_pnl); document.getElementById('shadow-unrealized').textContent=money(eq.unrealized_pnl??val.unrealized_pnl); document.getElementById('shadow-drawdown').textContent=eq.drawdown==null?'—':num(eq.drawdown*100)+'%';
  table('shadow-positions',['Symbol','Side','Qty','Entry','SL','TP','Status'],allPositions.map(p=>[p.symbol,p.side,p.quantity,p.entry_price||p.entry_fill_price,p.stop_loss_price||'—',p.take_profit_price||'—',p.status||'OPEN']));
  table('shadow-trades',['Exit','Symbol','Side','Strategy','Exit reason','Net P&L','Review'],trades.map(t=>[time(t.exit_time),t.symbol,t.side,t.strategy_family,t.exit_reason,money(t.net_pnl),t.review_status]));
  table('shadow-performance',['Side','Trades','Wins','Net P&L','Avg return'],(s.performance||[]).map(p=>[p.side,p.trades,p.wins,money(p.net_pnl),p.average_return==null?'—':num(p.average_return*100)+'%']));
  const summaries=s.trade_forward_summary||[]; table('shadow-evidence',['Strategy','Source','Observations','Accepted','Wins','Net P&L','First','Latest'],summaries.map(x=>[x.strategy_family,x.evidence_source,x.total_observations,x.accepted_observations,x.wins,money(x.net_pnl),time(x.first_observation),time(x.latest_observation)]));
  const bm=s.benchmark||{}; table('shadow-benchmark',['Benchmark','Price','Age sec','Status'],[[bm.symbol||'NSE:NIFTY50-INDEX',money(bm.price),num(bm.age_seconds,1),bm.status||(bm.fresh?'FRESH':'STALE')]]);
  ...
  table('stock-decisions',['Symbol','Regime','State','Action','Family','Reason'],(s.stock_decisions||[]).map(x=>[x.symbol,x.market_regime,x.stock_state,x.action,x.strategy_family||'—',x.reason]));
  const ops=s.operations||{}, bar=ops.last_completed_bar||{}, data=s.data_readiness||{}, auth=service.authentication||{}, fin=service.finalization||{}, pf=service.preflight||{};
  table('operations',['Field','Value'],[['Lifecycle',rec.lifecycle],['Token health',auth.state||'UNKNOWN'],['Daily data',data.state||'unavailable'],['Daily bars',data.minimum_completed_bars==null?'—':`${data.minimum_completed_bars}/${data.warmup_required}`],['Daily data reasons',(data.reasons||[]).join(', ')||'none'],['Dataset hash',data.dataset?.artifact_sha256],['Last completed bar',bar.session_date],['Missing symbols',(data.missing_symbols||bar.missing_symbols||[]).join(', ')||'none'],['Session finalization',fin.status||'pending'],['Preflight',pf.passed===true?'PASS':pf.passed===false?'FAIL':'not run'],['VM heartbeat',time(service.heartbeat?.updated_at)],['Disk free GiB',ops.disk_free_bytes==null?'—':num(ops.disk_free_bytes/1073741824,2)],['Off-site backup',ops.offsite_backup?.status||'not reported'],['Alert delivery',ops.alert_delivery?.status||'not reported']]);
