...
function render(d) {
  ...
  document.getElementById('state-badge').textContent=rec.running?'RECORDING':'MONITORING'; document.getElementById('state-badge').className='state-badge '+(rec.running?'running':'stopped');
  ...
  document.getElementById('qualification').textContent=ready.details?.qualification||'unavailable'; document.getElementById('backup-age').textContent=ready.details?.latest_backup?'available':'missing';
  ...
  table('orders',['ID','Broker ID','Status','Filled'],[...(s.orders||[]).map(o=>[o.intent_id,o.broker_id,o.status,o.filled]),...(s.scheduled_intents||[]).map(i=>[i.intent_id,'—','SCHEDULED','—'])]);
